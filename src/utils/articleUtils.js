...
async function findOrCreateTags(connection, tagCodes, languageCode) {
 ...
 // Utility: derive a simple MIME type from a URL's extension or provider
 // NOTE:
 // - We keep the default as "image/jpeg" to preserve existing behaviour where
 //   unknown URLs are treated as images.
 // - We add special handling for common video providers (e.g. YouTube) so that
 //   URLs like "https://www.youtube.com/watch?v=..." are recognised as video.
 const mimeFromUrl = (url) => {
   try {
     if (typeof url !== "string") return "image/jpeg";

     const lower = url.toLowerCase();
     const u = lower.split("?")[0].split("#")[0];

     // Provider-based detection (no reliable file extension)
     if (u.includes("youtube.com/") || u.includes("youtu.be/")) {
       // Treat any YouTube URL as a video
     ...
     // Fallback: keep legacy behaviour (unknown => image)
     return "image/jpeg";
   } catch {
     return "image/jpeg";
   }
